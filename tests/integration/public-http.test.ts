...
describe.skipIf(process.env.LOTSORA_TEST_HTTP !== "1")("N3 im echten Next-Produktionsserver", () => {
  ...
  beforeAll(async () => {
    ...
    proxy = createServer((incoming, outgoing) => {
      ...
      if (stall && path === "/rest/v1/rpc/get_published_product_pass") {
      ...
      if ((failTable || browserFault === "products") && path === "/rest/v1/rpc/get_published_product_pass") {
      ...
      const upstream = httpRequest(`${testConfig().url}${path}`, {
        ...
        const hold = heldRead;
        if (hold && path === "/rest/v1/rpc/get_published_product_pass") {
          const chunks: Buffer[] = [];
  ...
  it("teilt Seite/Metadaten: ein vollständiger Stand, keine Signierung; Folgeanfrage liest frisch", async () => {
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      expect(requests.filter(x => x.includes("/rest/v1/"))).toEqual(["POST /rest/v1/rpc/get_published_product_pass"]);
  ...
  it("frische Seite und Metadaten nach Änderung, Rücknahme und Wiederveröffentlichung", async () => {
    ...
    expect(content.html).toContain(`<title>${f.a.published.name} – Produktpass | lotsora</title>`);
  ...
  it("alte Seitenlinks prüfen beim Öffnen erneut; Redirects und Fehler sind no-store", async () => {
    ...
    try {
      ...
      expect((await f.a.client.from("documents").update({ visibility: "oeffentlich" }).eq("id", f.a.publicDoc.id)).error).toBeNull();
      expect((await publishFixtureProduct(f.a.client, f.a.published.id, true)).error).toBeNull();
    }
    expect((await fetch(url, { redirect: "manual" })).status).toBe(307);
  });
  it("DB-Ausfall ist weder Nichtexistenz noch stiller Teilpass; Metadaten neutral und Wiederholung erfolgreich", async () => {
    for (const table of ["get_published_product_pass"]) {
      failTable = table;
      try {
  ...
  it("hängende Abfrage endet begrenzt ohne automatische DB-Wiederholung", async () => {
    ...
    try {
      ...
      expect(requests.filter(x => x === "POST /rest/v1/rpc/get_published_product_pass")).toHaveLength(1);
    } finally { stall = false; }
    expect((await page()).html).toContain("HTTP-Neuer-Name");
  });
  it("dokumentiert laufende Lesung bei Rücknahme; nächste Anfrage ist sicher gesperrt", async () => {
    ...
    try {
      ...
      // Der vor Rücknahme vollständig gelesene Stand kann einen bereits
      // gestarteten Request noch abschließen.
