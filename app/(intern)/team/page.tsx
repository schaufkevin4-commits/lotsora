...
export default async function TeamPage() {
  ...
  return <div className="space-y-8">
    <div className="space-y-2">
      <p className="text-sm text-muted-foreground">{firma.company_name}</p>
      <h1 className="text-2xl font-semibold">Team</h1>
      <p className="text-muted-foreground">Gemeinsam an Produkten und digitalen Produktpässen arbeiten.</p>
    </div>
    <section aria-labelledby="members-heading" className="space-y-4">
      ...
      <ul className="divide-y rounded-lg border">
        {members.data.map(member => <li key={member.user_id} className="space-y-3 p-4">
          ...
          {owner && member.role !== "owner" && <div className="space-y-2">
    ...
    {owner ? <>
      <section aria-labelledby="invite-heading" className="space-y-4 rounded-lg border p-4 sm:p-6">
        <h2 id="invite-heading" className="text-lg font-semibold">Mitarbeiter einladen</h2>
